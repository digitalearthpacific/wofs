...
def _shade_row(shade_mask, elev_m, sun_alt_deg, pixel_scale_m, no_data, fuzz=0.0):
    ...
    shade_mask[0] = LIT
    shade_mask[1:] = numpy.where(
        (elev_m[:-1] - elev_m[1:]) / pixel_scale_m < tan_sun_alt, LIT, SHADED
    )
    ...
    for i in switch[
        0
    ]:  # note: could use flatnonzero instead of where; or else switch,=; to avoid [0]. --BL
        if shade_mask[i] == LIT:
            # TODO: horizontal fuzz?
            shadow_level = (elev_m[i] + fuzz) - numpy.arange(shade_mask.size - i) * (
                tan_sun_alt * pixel_scale_m
            )
...
def vector_to_crs(point, vector, original_crs, destination_crs):
    ...
    transformed_vector = tuple(
        map(lambda x: x[1] - x[0], zip(*transformed_line.points))
    )
    return transformed_point, transformed_vector


def solar_vector(point, time, crs):
    (lon, lat), (dlon, dlat) = vector_to_crs(
        point, (0, 100), original_crs=CRS(crs), destination_crs=CRS("EPSG:4326")
    )
...
def shadows_and_slope(tile, time, no_data=-1000):
    ...
    rotated_elv_array = ndimage.interpolation.rotate(
        buff_elv_array,
        rot_degrees,
        reshape=True,
    ...
    shadows = ndimage.interpolation.rotate(
        shadows,
        -rot_degrees,
        reshape=False,
        output=numpy.float32,
    ...
    shadows = xarray.DataArray(
        shadows.reshape(tile.elevation.shape), coords=tile.elevation.coords
    )
